...
@app.route('/education')
def education():
    # Legacy path — Education Programs is now branded as Nischint.
    # Permanent redirect keeps old links/bookmarks/SEO working.
    return redirect(url_for('nischint'), code=301)

@app.route('/nischint')
def nischint():
    _track(request, 'nischint')
    return render_template('nischint.html')
...
def admin_edit_program(pid):
    ...
    db = get_db()
    db.execute("""UPDATE programs SET title=?, category=?, description=?, tag=?, icon=?
                  WHERE id=?""",
               (title, category, description, tag, icon, pid))
    db.commit()
    return redirect(url_for('admin_programs'))
